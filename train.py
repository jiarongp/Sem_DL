...
#########################################################################################
TOTAL_BATCH = 2000
dataset_path = "D:/Coding/PYTHON/seqGAN2/data/movie/"
...
def main():
    ...
    pre_train_data_loader.create_batches([tweets_embedded.id])

    gen_data_loader = Gen_Data_loader(BATCH_SIZE, vocab_dict)
    gen_data_loader.create_batches(tweets_embedded.id)
    ...
    for _ in range(10):   # 10
        ...
        dis_data_loader.load_train_data([tweets_embedded.id], [negative_file])
        for _ in range(3):
